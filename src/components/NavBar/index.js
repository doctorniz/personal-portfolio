...
const NavBar = (props) => {
    ...
    const linkComponents = props.links.map((each,i) => (<li key={i}>
    ...
    return (<nav id="nav" className={`nav ${menuVisible ? 'open' : ''} ${isPageTop ? '' : 'sticky'}`}>
    ...
    <ul className="navbar">
      {linkComponents}
    <input className="navbar-input" placeholder="ENTER YOUR CODE" type="text" />
